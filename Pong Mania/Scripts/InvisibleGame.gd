...
func _process(delta):
	...
	if $InvisibleBall.position.x > screenSize.x:
		...
		$InvisibleBall.resetBall()
		updateScore()
	# Make the enemy paddle visible if the ball's velocity is pointed towards
	# the player and is near the enemy paddle (this suggests that the ball was
	# just bounced by the enemy paddle)	
	if ($InvisibleBall.position.x > screenSize.x - 80) and ($InvisibleBall.velocity.x < 1):
		$InvisiblePaddle.makeVisible()
